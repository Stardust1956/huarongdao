...
class NumberHuaRong(QWidget):
    ...
    def initUI(self):
        global zuhao
        i, flag = QInputDialog.getInt(self, "选择字母", "序号:", 0, 0, 35, 1)
        zuhao = i
        # 设置方块间隔
    ...
    def onInit(self):
        ...
        self.ans = ""
        self.count = 0
        self.disnum = random.randint(1, 9)  # 缺的数字
        self.numbers = list(range(1, 10))
            ...
            for row in range(3):
                for column in range(3):
                    ...
                    if temp == 0:
                        self.zero_row = row
                        self.zero_column = column
            QMessageBox.information(self, '强制交换', "进行强制交换"+"["+str(i)+" , "+str(j)+"]")

            self.updatePanel()
            while not judge(self.blocks, self.goal):
                text, ok = QInputDialog.getText(self, '现在无解', '请输入您的交换如1 2')
                x = []
                x.append(int(text[0]))
                x.append(int(text[2]))
                print(x)
...
class FirstUi(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn1 = QPushButton('简单游戏', self)
        ...
        self.btn2.setGeometry(365, 650, 225, 75)
        ...
        self.btn3 = QPushButton('困难游戏', self)
        self.btn3.setGeometry(365, 500, 225, 75)
        self.btn3.setStyleSheet("QPushButton{color:black;font-size:40px}"
                                "QPushButton:hover{background-color:lightgreen}"
                                "QPushButton{background-color:lightblue}"
                                "QPushButton{border:2px}"
                                "QPushButton{border-radius:10px}"
...
class SecondUi(QWidget):
    ...
    def init_ui(self):
        ...
        self.label = QLabel(self)
        self.label.setGeometry(225, 150, 500, 700)
        self.label.setText(
            "1.简单模式：将原始字符图片平均切割成九份小图，并随机抠掉一张图充当空格，此时图片为原始状态，然后我们将小图的顺序打乱并拼接回去，你需要做的事就是移动白色的图片将图片恢复到原始的状态\n\n2.困难模式：当你移动到50步的时候，我们会强制随机调换此时棋盘上的两个格子，由于此时棋盘不一定有解，所以我们给了你一次自由调换的机会，你可以调换任意两个图片的位置，注意这个自由调换只能在棋盘无解的情况下使用，且需紧接着强制调换的操作，如果调换完还是无解可以继续调换。\n\n3.按Z解题，按C进行AI提示")
